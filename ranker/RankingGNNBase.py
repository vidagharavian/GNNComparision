...
class RankingGNNBase(nn.Module):
    ...
    def __init__(self, embedding_dim: int, Fiedler_layer_num: int =3, alpha: float =0.01, trainable_alpha: bool =False,initial_score: Optional[torch.FloatTensor ] =None, prob_dim: int =5, sigma: float =1.0, **kwargs):
    ...
    def obtain_score_from_dist(self) -> torch.FloatTensor:
        self.score = torch.exp(-((self.z - self._anchor_vec).norm(2, 1 ) /torch.exp(self._log_sigma)) ** 2/ self.z.shape[1])[:,
                     None]  # Gaussian kernel to serve as similarity, range from 0 to 1
        return self.score
    ...
    def obtain_score_from_proximal(self, start_from: str = 'dist') -> torch.FloatTensor:
        ...
        if start_from == 'baseline':
            ...
            if start_from == 'dist':
                ...
                raise ValueError(
                    'Please input the correct start_from value from baseline, dist and innerproduct instead of {}!'.format(
                        start_from))
